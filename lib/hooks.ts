...
const handleShortlistMessage = (
  ...
  console.log("message data is", messageData, userId);
  if (messageData.id === userId && messageType === "movies-movieOfTheWeek") {
    console.log("got message for own shortlist", messageData, userId);
    queryClient.setQueryData(["shortlist", userId], (oldData: any) => {
      return produce(oldData, (draft: Shortlist) => {
        draft.movies = messageData.movies;
      });
    });
  }
...
const handleMessage = (
  ...
  switch (channelName) {
    case "movieclub-shortlist":
      handleShortlistMessage(eventName, data, queryClient, userId);
      break;
  }
};
    ...
    const bindPusher = async () => {
      ...
      channel.bind(eventName, (data: PusherMessage) => {
        if (channelName === "movieclub-raffle") {
          ...
          console.log("handling message", session?.user?.shortlistId);
          handleMessage(
            eventName,
            channelName,
            data,
