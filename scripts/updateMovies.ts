...
import { Image, TMDBMovieResponse } from "@/types/tmdb.type";
import type { Image as MovieImage } from "@/types/movie.type";
...
export default async function updateMovies() {
  ...
  for (const movie of movies) {
    ...
    let backdrops = data.images?.backdrops
    ...
    let posters = data.images?.posters
    ...
    let logos = data.images?.logos.filter(
